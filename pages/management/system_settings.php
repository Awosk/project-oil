...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['bildirim_kaydet'])) {
    ...
    if ($hedef_id) {
        ...
        $pdo->prepare("UPDATE users SET mail_notifications = ? WHERE id = ?")->execute([$mail_aktif, $hedef_id]);

        // Mevcut filtreleri sil
        $pdo->prepare("DELETE FROM notification_filters WHERE user_id = ?")->execute([$hedef_id]);

        // Yenilerini ekle
        $secili = $_POST['bildirim_filtreler'] ?? [];
        if (!empty($secili)) {
            $stmt = $pdo->prepare("INSERT INTO notification_filters (user_id, is_active, module, action) VALUES (?, 1, ?, ?)");
...
$adminler = $pdo->query("SELECT id, full_name AS ad_soyad, username AS kullanici_adi, email, mail_notifications AS mail_bildirim_aktif FROM users WHERE role = 'admin' AND is_active = 1 ORDER BY full_name")->fetchAll();

// Bildirim filtrelerini çek
$mevcut_filtreler = [];
$filtre_rows = $pdo->query("SELECT user_id, module, action FROM notification_filters WHERE is_active = 1")->fetchAll();
foreach ($filtre_rows as $row) {
    $mevcut_filtreler[$row['user_id']][$row['module'] . '|' . $row['action']] = true;
}
